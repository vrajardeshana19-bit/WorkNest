...
class LeaveService:
    ...
    def approve_leave(self, db: Session, leave_id: uuid.UUID) -> LeaveRequest:
    ...
    def reject_leave(self, db: Session, leave_id: uuid.UUID) -> LeaveRequest:
        leave = db.get(LeaveRequest, leave_id)
        if not leave:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Leave request not found.")
    ...
    def cancel_leave(self, db: Session, leave_id: uuid.UUID) -> LeaveRequest:
        leave = db.get(LeaveRequest, leave_id)
        if not leave:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Leave request not found.")
